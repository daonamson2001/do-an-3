  ...
  <body >
    ...
    <table>
    	<thead>
    		<tr>
    			<th width="200px">
            TỔNG CÔNG TY ĐIỆN LỰC <br> KHÁNH HÒA
    			<br>
    			  Đơn vị: Vĩnh Hải
    			</th>
    			<th width="300px">
    ...
    <table>
      <tr>
        <td width="120px"><strong>Nhân viên lập phiếu:</strong></td> <td>{{session('fullname')}}</td>
        <td><strong></td>
      </tr>
    </table><br><br>
        ...
        <?php $count = 0; ?>
            ...
            <tr width="5%">
              ...
              <td style="border:thin blue solid;border-style:dashed;" >
                  <?php  
                    $sup_name = DB::table('supplies')->where('id', $di->sup_id)->first();
                    print_r($sup_name->sup_name);
                  ?>
              </td>
              <td style="border:thin blue solid;border-style:dashed;" >
                  <?php  
                    $pro_name = DB::table('producers')->where('id',$di->pro_id)->first();
                    print_r($pro_name->pro_name);
                  ?>
              </td>
              <td style="border:thin blue solid;border-style:dashed;" >
                  <?php  
                    $qua_name = DB::table('qualities')->where('id',$di->qua_id)->first();
              ...
              <td style="border:thin blue solid;border-style:dashed;" >
                  <?php  
                    $unit_name = DB::table('units')->where('id', (DB::table('supplies')->where('id', $di->sup_id)->first())->unit_id)->first();
